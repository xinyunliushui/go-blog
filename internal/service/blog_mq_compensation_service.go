...
func processBlogCompensationBatch(repo repository.IMQCompensationRepository) {
	...
	for i := range rows {
		row := &rows[i]
		rowCtx := common.ContextWithTraceID(context.Background(), common.CompensationTraceID(row.TraceID, row.ID))
		...
		case model.TaskTypeConsume:
			// 处理消费补偿
			processConsumeCompensation(repo, rowCtx, row)
		default:
			// 处理发布补偿
			processPublishCompensation(repo, rowCtx, row)
		}
	}
}
 ...
 */
func processPublishCompensation(repo repository.IMQCompensationRepository, ctx context.Context, row *model.BlogMQCompensation) {
	logger := common.LoggerFromCtx(ctx)
	var blog model.Blog
	if err := json.Unmarshal(row.Payload, &blog); err != nil {
		handleCompensationRetry(repo, ctx, row, 0, err, "[MQ补偿-推送]")
		return
	}

	queue := config.Conf.Rabbitmq.QueueName
	// 重新投递 MQ
	if err := rabbitmq.PublishMessage(ctx, queue, &blog); err != nil {
		logger.Errorf("[MQ补偿-推送] id=%s blog_id=%s 第%d次重试仍失败: %v",
	...
	if err := repo.MarkSent(row.ID); err != nil {
		logger.Errorf("[MQ补偿-推送] 标记成功失败 id=%s: %v", row.ID, err)
		return
	}
	logger.Infof("[MQ补偿-推送] 重试成功 id=%s blog_id=%s", row.ID, row.BlogID)
}
 ...
 * @param row *model.BlogMQCompensation 补偿记录
 */
func processConsumeCompensation(repo repository.IMQCompensationRepository, ctx context.Context, row *model.BlogMQCompensation) {
	logger := common.LoggerFromCtx(ctx)
	var blog model.Blog
	if err := json.Unmarshal(row.Payload, &blog); err != nil {
		handleCompensationRetry(repo, ctx, row, row.EffectivePendingMask(), err, "[MQ补偿-消费]")
		return
	}
	...
	if err := ReconcileAndSyncBlog(&blog, mask); err != nil {
		...
		logger.Errorf("[MQ补偿-消费] id=%s blog_id=%s mask=%s 第%d次失败: %v next=%s",
			row.ID, row.BlogID, FormatSyncMask(mask), row.RetryCount+1, err, FormatSyncMask(nextMask))
		handleCompensationRetry(repo, ctx, row, nextMask, err, "[MQ补偿-消费]")
		return
	}

	if err := repo.MarkSent(row.ID); err != nil {
		logger.Errorf("[MQ补偿-消费] 标记成功失败 id=%s: %v", row.ID, err)
		return
	}
	logger.Infof("[MQ补偿-消费] 重试成功 id=%s blog_id=%s", row.ID, row.BlogID)
}
 ...
 */
func handleCompensationRetry(repo repository.IMQCompensationRepository, ctx context.Context, row *model.BlogMQCompensation, pendingMask uint8, cause error, logPrefix string) {
	logger := common.LoggerFromCtx(ctx)
	...
	if err := repo.MarkRetry(row.ID, next, errMsg, dead, pendingMask); err != nil {
		logger.Errorf("%s 更新补偿记录失败 id=%s: %v", logPrefix, row.ID, err)
		return
	}
	if dead {
		if row.TaskType == model.TaskTypeConsume {
			logger.Errorf("%s [MQ_CONSUME_DEAD] id=%s blog_id=%s mask=%s 请人工核对 ES/CH",
				logPrefix, row.ID, row.BlogID, FormatSyncMask(pendingMask))
		} else {
			logger.Errorf("%s [MQ_PUBLISH_DEAD] id=%s blog_id=%s 请人工补投 MQ", logPrefix, row.ID, row.BlogID)
		}
	}
}
